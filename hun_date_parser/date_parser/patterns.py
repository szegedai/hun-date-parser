...
R_TOLIG_YM = r"((?:\b\d{4}(?!\d)) ?(?:\bjan(?:\b|\.|u[aá]r){1}|\bfeb(?:\b|r\.|\.|ru[aá]r){1}|\bm[aá]r(?:\b|c\b|c\.|\.|cius){1}|\b[aá]pr(?:\b|\.|ilis){1}\b|m[aá]j(?:\b|\.|us){1}|\bj[uú]n(?:\b|\.|ius){1}|\bj[uú]l(?:\b|\.|ius){1}|\baug(?:\b|\.|usztus){1}|\bszep(?:t\b|t\.|\b|\.|tember){1}|\bokt(?:\b|\.|[oó]ber){1}|\bnov(?:\b|\.|ember){1}|\bdec(?:\b|\.|ember))) ?- ?((?:\b\d{4}(?!\d))? ?(?:\bjan(?:\b|\.|u[aá]r)|\bfeb(?:\b|r\.|\.|ru[aá]r)|\bm[aá]r(?:\b|c\b|c\.|\.|cius)|\b[aá]pr(?:\b|\.|ilis)|\bm[aá]j(?:\b|\.|us)|\bj[uú]n(?:\b|\.|ius)|\bj[uú]l(?:\b|\.|ius)|\baug(?:\b|\.|usztus)|\bszep(?:t\b|t\.|\b|\.|tember)|\bokt(?:\b|\.|[oó]ber)|\bnov(?:\b|\.|ember)|\bdec(?:\b|\.|ember)))"
...
R_TOLIG_Y = r"(\b\d{4}(?!\d)) ?- ?(\b\d{4}(?!\d))"

# hyper day level patterns
R_ISO_DATE = r'(\b\d{4}(?!\d))(?:[-\\/\. ] ?(1[0-2]|0?[1-9]))?(?:[-\\/\. ] ?(1[0-9]|2[0-9]|3[01]|0?[1-9]))?'
R_REV_ISO_DATE = r'\b(1[0-9]|2[0-9]|3[01]|0?[1-9])[-\\/\. ] ?(1[0-2]|0?[1-9])[-\\/\. ] ?(\b\d{4}(?!\d))'
R_NAMED_MONTH = r'\b(j[oöő]v[oöő].*?|tavaly.*?)?(\bjan(?:\b|\.|u[aá]r){1}|\bfeb(?:\b|r\.|\.|ru[aá]r){1}|\bm[aá]r(?:\b|c\b|c\.|\.|cius){1}|\b[aá]pr(?:\b|\.|ilis){1}|m[aá]j(?:\b|\.|us){1}|\bj[uú]n(?:\b|\.|ius){1}|\bj[uú]l(?:\b|\.|ius){1}|\baug(?:\b|\.|usztus){1}|\bszep(?:t\b|t\.|\b|\.|tember){1}|\bokt(?:\b|\.|[oó]ber){1}|\bnov(?:\b|\.|ember){1}|\bdec(?:\b|\.|ember))(?: ([1-3][0-9]|[1-9]|egy.{0,5}|kett[őöo].{0,5}|h[aá]rom.{0,5}|n[eée]gy.{0,5}|öt.{0,5}|hat.{0,5}|h[eé]t.{0,5}|nyolc.{0,5}|kilenc.{0,5}|tíz.{0,5}|tizen.{0,10}|huszon.{0,10}|harminc.{0,10}|elsej[eé].{0,5}|második[aá].{0,5}|harmadik[aá].{0,5}|negyedik[eé].{0,5}|ötödik[eé].{0,5}|hatodik[aá].{0,5}|hetedik[eé].{0,5}|nyolcadik[aá].{0,5}|kilencedik[eé].{0,5}|tizedik[eé].{0,5}|tizen.{0,10}|huszadik[aá].{0,5}|huszon.{0,10}|harmincadik[aá].{0,5}))?'
R_RELATIVE_MONTH = r'(?:(\blegut[oó]bbi|\butols[oó]|\bmúlt|\bmult|\but[oó]bbi|\bezen|\bebben|\baktu[aá]lis|\baktuals|\bj[oöő]v[oöő]|\bjovo|\bk[oö]vetkez[oőö]|\bkovetkezo|\bk[oö]vetkezend[oőö]|\bel[oő]z[oő]|\belozo)(?!\s+(?:egy|kett[őöo]|két|h[aá]rom|n[eé]gy|[öo]t|hat|h[eé]t|nyolc|kilenc|t[íi]z|[0-9]+)).*)? a?(?:h[oó]nap|honap)(?:ban|ra|ig|t[oóöő]l|tol|om|unk|atok|uk)?'
R_NAMED_MONTH_SME = r"(\b\d{4}(?!\d)|j[oöő]v[oöő].*?|tavaly.*?)? ?(\bjan(?:\b|\.|u[aá]r){1}|\bfeb(?:\b|r\.|\.|ru[aá]r){1}|\bm[aá]r(?:\b|c\b|c\.|\.|cius){1}|\b[aá]pr(?:\b|\.|ilis){1}\b|m[aá]j(?:\b|\.|us){1}|\bj[uú]n(?:\b|\.|ius){1}|\bj[uú]l(?:\b|\.|ius){1}|\baug(?:\b|\.|usztus){1}|\bszep(?:t\b|t\.|\b|\.|tember){1}|\bokt(?:\b|\.|[oó]ber){1}|\bnov(?:\b|\.|ember){1}|\bdec(?:\b|\.|ember)) (elej|k[oö]zep|v[eé]g)"

R_WEEKDAY = r'(?:(el[oő]z[oő]|m[uú]lt|ezen|j[oöő]v[oöő]).*)?(h[eé]tf[oő](?:n|ön|re|ig|t[oóöő]l)?|kedd(?:en|re|ig|t[oóöő]l)?|szerd[aá](?:n|ra|ig|t[oóöő]l)?|cs[uü]t[oö]rt[oö]k(?:ön|re|ig|t[oóöő]l)?|p[eé]ntek(?:en|re|ig|t[oóöő]l)?|szombat(?:on|ra|ig|t[oóöő]l)?|vas[aá]rnap(?:on|ra|ig|t[oóöő]l)?)'
...
R_DAYNAME = r'\b(elsej[eé][a-z]*|m[aá]sodik[aá][a-z]*|harmadik[aá][a-z]*|negyedik[eé][a-z]*|[oö]t[oö]dik[eé][a-z]*|hatodik[aá][a-z]*|hetedik[eé][a-z]*|nyolcadik[aá][a-z]*|kilencedik[eé][a-z]*|tizedik[eé][a-z]*|tizenegyedik[eé][a-z]*|tizenkettedik[eé][a-z]*|tizenharmadik[aá][a-z]*|tizennegyedik[eé][a-z]*|tizen[oö]t[oö]dik[eé][a-z]*|tizenhatodik[aá][a-z]*|tizenhetedik[eé][a-z]*|tizennyolcadik[aá][a-z]*|tizenkilencedik[eé][a-z]*|huszadik[aá][a-z]*|huszonegyedik[eé][a-z]*|huszonkettedik[eé][a-z]*|huszonharmadik[aá][a-z]*|huszonnegyedik[eé][a-z]*|huszon[oö]t[oö]dik[eé][a-z]*|huszonhatodik[aá][a-z]*|huszonhetedik[eé][a-z]*|huszonnyolcadik[aá][a-z]*|huszonkilencedik[eé][a-z]*|harmincadik[aá][a-z]*|harmincegyedik[eé][a-z]*)'
