...
def extract_datetime_within_interval(interval_start: datetime,
    ...
    for r in res:
        if not (type(r['start_date']) == datetime and type(r['start_date']) == datetime):
            return ExtractWithinRangeSuccess.OPEN_RANGE_FALLBACK, text2datetime(query_text, now=fallback_now)

        if not (interval_start <= r['start_date'] and r['end_date'] <= interval_end):
            # Extracted datetime is out of expected interval...
            return ExtractWithinRangeSuccess.OUT_OF_RANGE_FALLBACK, text2datetime(query_text, now=fallback_now)
    ...
    if is_relative_datetime(query_text):
        ...
        return ExtractWithinRangeSuccess.RELATIVE_TIME_WORD_FALLBACK, text2datetime(query_text, now=fallback_now)
    else:
